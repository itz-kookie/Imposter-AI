...
import type { BotClueRequest, BotVoteRequest, Clue, Player } from "../../shared/game";
...
export default function App() {
  ...
  async function runBotTurn(bot: GamePlayer, pack: WordPack) {
    ...
    catch (cause) { setError(cause instanceof Error ? cause.message : "Could not reach Gemini"); botTurnKey.current = ""; }
  ...
  async function confirmVote() {
    ...
    const responses = await Promise.allSettled(bots.map(async bot => {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">◐</span><span>IMPOSTER</span><span className="prototype-pill">Gemini edition</span></div><button className="icon-btn" aria-label="How to play" onClick={() => alert("Know the word, give subtle clues, then vote for the imposter. A tied vote lets the imposter escape.")}>?</button></header>
...
function GameLayout({ children, players, order, activeId, doneCount = 0 }: { children: ReactNode; players: GamePlayer[]; order: string[]; activeId: string; doneCount?: number }) { return <div className="game-layout"><article className="game-card">{children}</article><aside className="side-card"><h3>At the table</h3><div className="players-list">{order.map((id, index) => { const p = players.find(x => x.id === id)!; return <div key={id} className={`player-row ${activeId === id ? "active" : ""} ${index < doneCount ? "done" : ""}`}><Avatar player={p} /><span><b>{p.name}</b><small>{activeId === id ? "Choosing a clue…" : index < doneCount ? "Clue given" : `Position ${index + 1}`}</small></span>{activeId === id && <i className="turn-dot" />}</div>; })}</div></aside></div>; }
...
function BotWaiting({ player, busy, error, onRetry, onFallback }: { player: GamePlayer; busy: boolean; error: string; onRetry: () => void; onFallback: () => void }) { return <div className="waiting"><div><div className="waiting-orb" /><h3>{player.name} is thinking</h3><p>{busy ? "Gemini is reviewing every clue…" : "Ready for the next clue."}</p>{error && <div className="error-box"><span>{error}</span><div><button className="secondary-btn compact" onClick={onRetry}>Retry Gemini</button><button className="text-btn" onClick={onFallback}>Use fallback clue</button></div></div>}</div></div>; }
